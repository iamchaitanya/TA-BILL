...
const App = () => {
  ...
  useEffect(() => {
    ...
    if (savedProfile) {
      try { setProfile(prev => ({ ...prev, ...JSON.parse(savedProfile) })); } catch (e) {}
    }
  }, []);
  ...
  const reportEntries = useMemo(() => {
    return data.entries.filter(e => e.lastSavedAt && e.date.startsWith(selectedMonthYear)).sort((a, b) => a.date.localeCompare(b.date));
  ...
  return (
    <div className="min-h-screen bg-[#fdfcfb] selection:bg-teal-100 flex flex-col">
      ...
      <header className="sticky top-0 z-30 bg-white/80 backdrop-blur-md border-b border-teal-100 p-4 no-print">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="bg-teal-600 text-white p-2 rounded-xl shadow-lg rotate-3"><svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg></div>
            <div><h1 className="text-xl font-black text-teal-900 leading-none">IotaBill</h1><p className="text-[10px] text-teal-600 font-bold uppercase tracking-widest">Logs</p></div>
          </div>
          <button onClick={() => setShowCalendar(true)} className="p-2 text-slate-400 hover:text-teal-600 transition-colors"><svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg></button>
        </div>
      </header>

      <main className="flex-1 max-w-7xl mx-auto w-full p-4 md:p-8 pb-24 no-print">
        {activeTab === 'entries' && (
          <EntriesPage 
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-teal-100 flex justify-around items-center py-2 px-6 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] no-print">
        ...
        <button onClick={() => setActiveTab('report')} className={`flex flex-col items-center gap-1.5 p-2 rounded-2xl transition-all ${activeTab === 'report' ? 'text-teal-600 bg-teal-50/50' : 'text-slate-400'}`}>
      ...
      {showCalendar && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm no-print">
          <div className="bg-white rounded-[40px] shadow-2xl w-full max-md overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-8 bg-teal-900 text-white flex justify-between items-center">
              <button onClick={() => changeMonth(-1)} className="p-2 hover:bg-white/10 rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M15 19l-7-7 7-7" /></svg></button>
              ...
              <button onClick={() => changeMonth(1)} className="p-2 hover:bg-white/10 rounded-full"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M9 5l7 7-7 7" /></svg></button>
            </div>
            <div className="p-8">
              <div className="grid grid-cols-7 gap-2 mb-4">
                {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, idx) => (<div key={d + idx} className={`text-center text-[10px] font-black py-1 ${idx === 0 ? 'text-red-500' : 'text-slate-300'}`}>{d}</div>))}
                {calendarDays.days.map((day, idx) => {
                  if (!day) return <div key={`empty-${idx}`} className="aspect-square" />;
                  const isRedDay = isHoliday(day);
                  return (<div key={idx} className={`aspect-square flex flex-col items-center justify-center rounded-xl text-xs font-black transition-all ${isRedDay ? 'text-red-500 bg-red-50/50' : 'text-slate-600 cursor-pointer'}`}>{day.getDate()}</div>);
                })}
              </div>
              <button onClick={() => setShowCalendar(false)} className="w-full mt-4 py-4 bg-slate-900 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest">Close</button>
            </div>
          </div>
        </div>
      )}
